...
class TasksManagementSystem {
    ...
    public static void main(String[] args) {
        ...
        if(tasks.size()>0)taskIdCounter = (tasks.get(tasks.size() - 1)).getTaskId() + 1;
    ...
    private static void addTask(Scanner scanner, List<Task> tasks) {
        ...
        tasks.add(new Task(taskIdCounter++, name, false));
    ...
    static class SortById implements Comparator<Task> {
        public int compare(Task t1, Task t2) {
            if (t1.getTaskId() == t2.getTaskId()) return 0;
            else if (t1.getTaskId() > t2.getTaskId()) return 1;
            else return -1;
        }
    }
    ...
    private static List<Task> loadTasksFromFile() {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(FILE_NAME))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                tasks.add(new Task(Integer.parseInt(data[0]), data[1],
    ...
    private static void saveTasksToFile(List<Task> tasks) {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(FILE_NAME))) {
            for (Task task : tasks) {
                writer.write(task.getTaskId() + DELIMITER + task.getTaskName() + DELIMITER +
                        task.getIsComplete());
                writer.newLine();
            }
        } catch (IOException e) {
...
class Task {
    ...
    public Task(int taskId, String taskName, boolean isComplete) {
        this.taskId = taskId;
    ...
    public String toString() {
        if (isComplete)
            return taskId + " " + taskName + " " + "Task is completed";
        return taskId + " " + taskName + " " + "Task is not completed";
    }
}
